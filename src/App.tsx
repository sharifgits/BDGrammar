...
export default function App() {
  ...
  const handleAIExtraction = (text: string) => {
    if (!text || !text.trim()) {
      alert("No text to extract. Please make sure the source material is valid.");
      return;
    }
      ...
      <AnimatePresence>
        ...
        {viewingGrammar !== null && (
          <GrammarExplanation
            ...
            customData={viewingGrammar.isCustom ? {
              ...customTopics.find(t => t.steps.some((s: any) => s.topicId === viewingGrammar.id))?.grammarData,
              title: customTopics.find(t => t.steps.some((s: any) => s.topicId === viewingGrammar.id))?.grammarData?.title || customTopics.find(t => t.steps.some((s: any) => s.topicId === viewingGrammar.id))?.title || "Custom Lesson"
            } : undefined}
            onClose={handleCloseGrammar}
            onStartPractice={handleStartLesson}
            onUpdateContent={async (newData) => {
              const topic = customTopics.find(t => t.steps.some((s: any) => s.topicId === viewingGrammar.id));
              if (topic) {
                const updatedTopic = { ...topic, grammarData: newData };
                ...
                <SmartCreator 
                  ...
                  onLessonCreated={(lesson) => {
                    setCustomTopics(prev => {
                      const existingIdx = prev.findIndex(t => t.id === lesson.id);
                      if (existingIdx >= 0) {
                        const copy = [...prev];
                        copy[existingIdx] = lesson;
                        return copy;
                      }
                      return [...prev, lesson];
                    });
