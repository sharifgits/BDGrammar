...
import { Sparkles, Loader2, BookmarkPlus, BookmarkCheck, Search, RefreshCw, Bookmark, Trash2, Library, Copy, Check, Download } from 'lucide-react';
...
Extract vocab list: word, pronunciation(Bengali), meaning(Bengali). JSON.`;
        const storyResp = await callGemini({
          model: "gemini-2.5-flash",
...
Return JSON array with one object.`;
  ...
  const handleDownloadImage = async (storyId: string, title: string) => {
    ...
    try {
        ...
        const dataUrl = await toPng(node, {
            ...
            pixelRatio: 2, // Reduced from 3 to 2 for better mobile performance
            cacheBust: true,
        });
  ...
  const renderStoryText = (text: string, vocabList: {word:string}[]) => {
    // Basic cleaning of markdown artifacts and stars
    ...
    return wordsInStory.map((wordObj, i) => {
        ...
        if (vocabLower.includes(cleanWord)) {
            return (
              <span key={i} className="text-red-600 dark:text-red-400 font-bold px-1 mx-0.5 transition-colors italic">
                {wordObj}
              </span>
            );
        }
  ...
  const renderStoryCard = (story: VocabStory, isSaved: boolean) => (
      <div id={`story-card-${story.id}`} key={story.id} className="bg-gradient-to-br from-indigo-50/20 to-rose-50/20 dark:from-slate-900 dark:to-slate-950 border-2 border-slate-100 dark:border-slate-800 rounded-3xl p-6 sm:p-10 shadow-sm overflow-hidden min-h-[450px] flex flex-col">
        ...
        <div className="text-slate-700 dark:text-slate-200 text-lg md:text-xl leading-[1.8] mb-12 border-b border-indigo-100 dark:border-slate-800 pb-10 font-serif text-justify px-2 drop-shadow-sm tracking-wide">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 mb-2">
            {story.vocabulary.map((v, i) => (
                <div key={i} className="flex flex-row items-start gap-3 p-3 bg-white/40 dark:bg-slate-800/40 rounded-xl border border-indigo-50/50 dark:border-slate-700/50 shadow-sm">
                    <span className="flex items-center justify-center w-6 h-6 rounded-full bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-[10px] font-black shrink-0 mt-0.5">
                      {i + 1}
                    </span>
                    <div className="flex flex-col gap-1 overflow-hidden">
                      <div className="flex items-baseline gap-2 flex-wrap">
                        <span className="text-red-600 dark:text-red-400 font-black text-base">{v.word}</span>
                        <span className="text-slate-400 dark:text-slate-500 text-[10px] font-bold tracking-wider italic">({v.pronunciation})</span>
                      </div>
                      <span className="text-slate-600 dark:text-slate-300 text-sm font-bold">{v.meaning}</span>
                    </div>
                </div>
            ))}
        </div>
        ...
        {!isSaved && (
            <div className="mt-auto pt-8 flex justify-end gap-2 flex-nowrap overflow-x-auto border-t border-indigo-50 dark:border-slate-800/10 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                <button 
                    onClick={() => handleDownloadImage(story.id, story.title)}
                    disabled={isDownloading === story.id}
                    className="px-3 sm:px-4 py-2 border-2 border-indigo-100 dark:border-slate-800 text-indigo-600 dark:text-indigo-300 hover:bg-white dark:hover:bg-slate-800 font-black rounded-xl transition-all flex items-center gap-1.5 text-[9px] sm:text-[10px] uppercase tracking-widest active:scale-95 disabled:opacity-50 whitespace-nowrap shrink-0"
                >
                    <Download size={14} />
                    Download
                </button>
                 ...
                 <button 
                    ...
                    className="px-3 sm:px-4 py-2 border-2 border-indigo-100 dark:border-slate-800 text-indigo-600 dark:text-indigo-300 hover:bg-white dark:hover:bg-slate-800 font-black rounded-xl transition-all flex items-center gap-1.5 text-[9px] sm:text-[10px] uppercase tracking-widest active:scale-95 whitespace-nowrap shrink-0"
                >
                 ...
                 <button 
                    ...
                    className={classNames(
                        "px-4 sm:px-5 py-2 rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest transition-all active:scale-95 flex items-center gap-1.5 whitespace-nowrap shrink-0",
